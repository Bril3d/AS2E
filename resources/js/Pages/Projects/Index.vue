...
<template>
  ...
  <AuthanticatedLayout title="Project List">
    <Link as="button" :href="route('projects.create')"
      ...
      <Table>
        ...
        <template #default>
          <tr v-for="project in projects.data" :key="project.id">
            ...
            <TableDataCell class="text-ellipsis overflow-hidden max-w-64">{{ project.title }}</TableDataCell>
            <TableDataCell>{{ project.user?.name }}</TableDataCell>
            <TableDataCell><img v-if="project.image" class="h-16 object-cover cursor-pointer" :src="project.image"
                :alt="project.title" @click="openFile(project.image)" /><span v-else>--</span>
            </TableDataCell>
            <TableDataCell class="text-ellipsis overflow-hidden max-w-64"><Link :href="route('projects.show',project)">Click to view</Link></TableDataCell>
            <TableDataCell>
              <div class="flex items-center gap-4 ">
                <Link as="button" :href="route('projects.edit', project.slug)"
                ...
                <Link as="button" :href="route('projects.destroy', project.slug)" method="delete" preserve-scroll
          ...
          <EmptyTable v-if="projects.data.length < 1" section="projects" />
        </template>
      </Table>
