        ...
        <div>
          ...
          clickable: true,
            ...
            <swiper-slide v-for="(image, index) in carousel" :key="index">
              ...
              <img :src="image.startsWith('http') ? image : `/storage/${image}`" :alt="image" class="h-screen object-cover w-full" />
            </swiper-slide>
          </swiper-container>

          <!-- Overlay for Text and Logo -->
          <div class="absolute top-1/2 sm:left-10 max-w-full md:left-20 z-10 sm:mr-2">
            <div class="text-white text-5xl sm:text-7xl text-center sm:text-left py-4">{{ data.title }}</div>
            <div class="bg-main text-white text-3xl sm:text-5xl font-bold  mix-blend-screen px-10 py-5 ">{{ data.subtitle }}</div>
          </div>
        </div>
      </template>
